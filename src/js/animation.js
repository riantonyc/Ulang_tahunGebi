    ...
    if (button.classList.contains("switch")) {
      ...
      if (!msg || msg.innerHTML.trim() === "") {
        ...
        setTimeout(() => {
          ...
          // Show modal after birthday card appears
          setTimeout(showTraktirModal, 1500);
        }, 1500);
        return;
      }
      ...
      // Next button - skip scroll and show birthday card immediately
      const nextBtn = document.getElementById("nextBtn");
      if (nextBtn) {
        nextBtn.addEventListener("click", function () {
          // Mark button as skipped
          nextBtn.classList.add("skipped");
          nextBtn.innerHTML = '<span>Selesai</span>';

          // Immediately complete the scroll animation
          msg.style.transition = "transform 0.5s ease";
          msg.style.transform = "translateY(-100%)";
          flash.style.display = "none";

          // Immediately show birthday card
          msgWindow.classList.add("fade-in");
          msgWindow.style.opacity = "0";
